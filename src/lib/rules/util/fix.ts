...
import { StringCompareOptions, stringCompare } from "@adashrodEps/lib/rules/util/strings";
...
export function fixUnsortedKeysWithComments(
    ...
    let addTrailingCommaBeforeSort = false;
    if (restOperatorToken !== null) {
        ...
        trailingCommaToken = findPunctuatorBetween(tokens,
            ...
            ",");
        addTrailingCommaBeforeSort = trailingCommaToken === null;
    }
    // I could remove the added trailing comma after sort, but that would require mapping each key to an object that
    // encapsulates key name, comma, comments, etc and comma style can just be enforced by another lint rule
    // or a formatter
        ...
        keys.slice()
            .map((key: AssignmentProperty | GenericImportSpecifier, index: number) =>
                sourceCode.getText(key) +
                    (index + 1 === keys.length && addTrailingCommaBeforeSort ? "," : "") +
            ...
            .sort((keyStringA, keyStringB) => stringCompare(keyStringA, keyStringB, options))
            .join("")
    );
}
